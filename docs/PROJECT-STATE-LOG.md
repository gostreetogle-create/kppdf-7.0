...
### PSL-012 — BOM domain: Material + Module + WorkType + Employee [2026-07-04]

| Поле | Значение |
|---|---|
| **Дата** | 2026-07-04 |
| **ID** | PSL-012 |
| **Тип** | `schema` (4 новые сущности + 1 product extension) + `process` (новая область) |
| **Модуль** | `backend` (новые модули `materials/`, `modules/`, `work-types/`, `employees/`) + `frontend` (4 новые страницы + product extension) |
| **Автор** | Buffy / MM3 |
| **Связанные OQ / PSL** | docs/backend/schemas/04-bom.md, docs/backend/DOMAIN-MODEL.md, docs/backend/BUSINESS-RULES.md §7-§10, docs/backend/ARCHITECTURE.md §2, docs/backend/RBAC-SCHEME.md |
| **Описание** | **BOM-домен зафиксирован.** 4 новые сущности + extension Product (status, productModuleIds). Иерархия: Product → Module → (Material \| Module \| Work). Цена каскадно вычисляется live через `ModuleService.computeCost(id)`.\n\n**Backend (4 модуля × ~3 файла + DTOs + схемы):**\n- `backend/src/modules/materials/` — schema + service + controller + DTOs\n- `backend/src/modules/modules/` — schema + service + controller + DTOs (с compute-cost endpoint)\n- `backend/src/modules/work-types/` — schema + service + controller + DTOs\n- `backend/src/modules/employees/` — schema + service + controller + DTOs\n- `backend/src/modules/products/schemas/product.schema.ts` — extension: `status` (DRAFT\|READY\|IN_PRODUCTION\|COMPLETED\|ARCHIVED), `productModuleIds: ObjectId[]`\n- `backend/src/common/types/permission-keys.ts` — +12 ключей: `MATERIALS_*` (3), `MODULES_*` (3), `WORKTYPES_*` (3), `EMPLOYEES_*` (3)\n- `backend/src/bootstrap/admin-seed.ts` — seed 3 базовых WorkType (Сварка 500₽/ч, Покраска 400₽/ч, Сборка 350₽/ч)\n\n**Frontend (4 страницы + 1 extension):**\n- `frontend/src/app/pages/admin/admin-tabs.ts` — nav groups «Каталог» (Materials + Modules) + «Справочники» (WorkTypes + Employees)\n- `frontend/src/app/services/api.service.ts` — новые типы: `Material`, `Module`, `WorkType`, `Employee`, `ModuleMaterial`, `ModuleWork`, `ProductModule`, enum'ы. CRUD-методы для всех 4 + `computeCost(id)`\n- `frontend/src/app/pages/admin/admin-loader.ts` — load-стримы для 4 новых сущностей\n- `frontend/src/app/pages/admin/admin.component.html` — render-блоки для 4 новых табов + quick-create dropdown'ы (supplier / material)\n- `frontend/src/app/pages/admin/admin.component.css` — стили для статусов-бейджей, drag-drop ordering\n- `frontend/src/app/pages/admin/admin.component.ts` — расширение `activeTab` union + методы CRUD для 4 сущностей + computeCost handler\n\n**Деferrred (в отдельных фазах):**\n- Phase X.1: Storage / StockMovement (multi-warehouse) — BR-MAT-stock\n- Phase X.2: Order + Gantt + auto-distribute works to free employee slots\n- Phase X.3: Module nesting UI polish (3+ levels rendering)\n- Phase X.4: Material offcut tracking (Variant 3, full склад) |
| **Причина** | PO запросил иерархию Product → Module → Material с поддержкой работ (для будущего Gantt), поставщиками материалов, возможностью standalone-модулей. Детальный vision + 4 уточняющих вопроса заданы через `ask_user`; все ответы в этой записи. Грамотный modular design: маленькие сущности, готовые к расширению. |
| **Затронутые файлы** | 🆕 Backend (~22 файла):\n• `backend/src/modules/materials/{materials.module,materials.controller,materials.service,dto/create-material.dto,dto/update-material.dto,schemas/material.schema}.ts`\n• `backend/src/modules/modules/{modules.module,modules.controller,modules.service,dto/create-module.dto,dto/update-module.dto,schemas/module.schema}.ts`\n• `backend/src/modules/work-types/{work-types.module,work-types.controller,work-types.service,dto/create-work-type.dto,dto/update-work-type.dto,schemas/work-type.schema}.ts`\n• `backend/src/modules/employees/{employees.module,employees.controller,employees.service,dto/create-employee.dto,dto/update-employee.dto,schemas/employee.schema}.ts`\n\n📝 Изменены (~9):\n• `backend/src/common/types/permission-keys.ts` (+12 keys)\n• `backend/src/modules/products/schemas/product.schema.ts` (+status, +productModuleIds)\n• `backend/src/modules/products/dto/product.dto.ts` (+status, +productModuleIds)\n• `backend/src/modules/products/products.service.ts` (+addModule, +removeModule, +reorderModules)\n• `backend/src/modules/products/products.controller.ts` (+addModule, +removeModule endpoints)\n• `backend/src/modules/products/products.module.ts` (imports MongooseModule forFeature on Materials/Modules/WorkTypes/Employees for refs)\n• `backend/src/app.module.ts` (+4 new module imports)\n• `backend/src/bootstrap/admin-seed.ts` (+3 WorkType seed)\n• `frontend/src/app/services/api.service.ts` (+types, +methods)\n• `frontend/src/app/pages/admin/admin-tabs.ts` (+nav groups)\n• `frontend/src/app/pages/admin/admin-loader.ts` (+load streams)\n• `frontend/src/app/pages/admin/admin.component.html` (+4 tab blocks)\n• `frontend/src/app/pages/admin/admin.component.ts` (+CRUD methods, +computeCost)\n• `frontend/src/app/pages/admin/admin.component.css` (+status badges, +drag-drop)\n\n📝 Docs (~6):\n• `docs/backend/schemas/04-bom.md` (new)\n• `docs/backend/DOMAIN-MODEL.md` (1.2→1.3, +4 entities)\n• `docs/backend/BUSINESS-RULES.md` (2.1→2.2, +§7-§10)\n• `docs/backend/ARCHITECTURE.md` (1.1→1.2, +4 module folders)\n• `docs/backend/RBAC-SCHEME.md` (planned, +12 permission keys)\n• `docs/PROJECT-STATE-LOG.md` (1.5→1.6, эта запись) |
...
| Версия | Дата | Что |
|---|---|---|
| 1.6 | 2026-07-04 | Добавлена запись **PSL-012** — BOM domain (Material + Module + WorkType + Employee). 4 новые сущности, 12 новых permission keys, ~22 новых backend-файла, ~5 frontend-файлов, ~6 docs-файлов. Иерархия Product → Module → (Material \| Work \| Module). См. [`schemas/04-bom.md`](../docs/backend/schemas/04-bom.md). |
